...
export async function sendSubmissionToTaskGrader(submissionId: string, submissionData: SubmissionParameters) {
    ...
    if (answerTokenParams && !process.env.TEST_MODE) {
        ...
        const decodedAnswer = JSON.parse(answerTokenParams.sAnswer);
