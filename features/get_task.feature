Feature: Get task
  ...
  Background: Create task
    ...
    And the database has the following table "tm_tasks_strings":
      | ID   | idTask    | sLanguage  | sTitle  | sTranslator | sStatement          | sSolution       | iVersion   |
      | 3000 | 1000      | fr         | Plateau |             | <p>Instructions</p> | <p>Solution</p> | 2147483647 |
    ...
    And the database has the following table "tm_source_codes":
      | ID   | idTask | sDate      | sParams                | sName              | sSource      | bEditable | bSubmission | sType | bActive | iRank | iVersion   |
      | 7001 | 1000   | 2023-04-03 | {"sLangProg":"python"} | defaultSource      | print("ici") | 0         | 0           | Task  | 1       | 0     | 2147483647 |
  ...
  Scenario: Get task by id with logged token
